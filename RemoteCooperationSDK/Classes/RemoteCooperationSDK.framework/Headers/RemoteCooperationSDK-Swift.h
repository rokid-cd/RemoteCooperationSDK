...
/// \param failure 失败回调
///
- (void)initWithConfigWithAppId:(NSString * _Nonnull)appId rtcUrl:(NSString * _Nonnull)rtcUrl saasUrl:(NSString * _Nonnull)saasUrl success:(void (^ _Nonnull)(id _Nullable))success failure:(void (^ _Nonnull)(NSError * _Nullable))failure SWIFT_METHOD_FAMILY(none);
...
- (void)setEvaluatorHostsWithHost:(NSString * _Nonnull)host;
- (void)showRKWindow;
@end
...
- (void)previewFileWithPaths:(NSArray<NSString *> * _Nonnull)paths index:(NSInteger)index type:(enum RKMeetingFileType)type;
- (void)showToastWithText:(NSString * _Nonnull)withText inView:(UIView * _Nullable)inView;
- (void)showToastWithText:(NSString * _Nonnull)withText inView:(UIView * _Nullable)inView completion:(void (^ _Nullable)(BOOL))completion;
- (void)showLoadingInView:(UIView * _Nullable)inView;
- (void)hideLoadingInView:(UIView * _Nullable)inView;
@end
...
/// \param isFinish 是否结束
///
- (void)getMeetingShareStatusWithChannelId:(NSString * _Nonnull)channelId shareType:(enum ShareType)shareType isFinish:(BOOL)isFinish;
/// 重新加入按钮点击回调
- (void)rejoinAction;
/// 创建会议按钮点击回调
- (void)startCooperationAction;
/// 离开会议回调
- (void)leaveMeetingAction;
@end
